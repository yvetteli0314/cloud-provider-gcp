...
func (ca *cloudCIDRAllocator) updateCIDRAllocation(nodeName string) error {
	...
	multiNetworkingDisabled := len(instance.NetworkInterfaces) == 1 &&
		(len(instance.NetworkInterfaces[0].AliasIpRanges) == 1 ||
			ca.cloud.GetIPV6Address(instance.NetworkInterfaces[0]) != nil)

	// get node default labels for multi-networking enabled clusters
	// - if node labels exist, get cidrStrings directly and update Node.Spec.PodCIDR(s)
	// - Otherwise, keep the old way using performMultiNetworkCIDRAllocation to
	// get cidrStrings by comparing the “default” Network against the GNP object
	hasNodeLabels, defaultSubnet, defaultPodRange := getNodeDefaultLabels(node)

	if multiNetworkingDisabled {
		...
		if hasNodeLabels {
			cidrStrings = ca.getDefaultNwCIDRs(node, instance.NetworkInterfaces, defaultSubnet, defaultPodRange)
		}
	}

	// update Node.Spec.PodCIDR(s) right away with cidrStrings
	if multiNetworkingDisabled || !multiNetworkingDisabled && hasNodeLabels {
		if err = ca.updateNodePodCIDRWithCidrStrings(oldNode, node, cidrStrings); err != nil {
			return err
		}
	}
	// update Node.Spec.PodCIDR(s) after performing multiNetworkCIDRAllocation
	if !multiNetworkingDisabled {
