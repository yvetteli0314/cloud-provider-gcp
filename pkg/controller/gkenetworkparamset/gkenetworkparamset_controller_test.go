...
func TestSetDesiredDefaultParamSet(t *testing.T) {
	desiredDefaultParamSet := newL3GNP(networkv1.DefaultPodNetworkName, []string{defaultPodRange})
			...
			defaultParamSet: &networkv1.GKENetworkParamSet{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						labelsAddonManagerMode: addonManagerMode,
					},
				},
				Spec: networkv1.GKENetworkParamSetSpec{
					VPC:           "random-vpc",
...
func TestSyncDefaultPodRanges(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "No node",
			defaultParamSet: newL3GNP(networkv1.DefaultPodNetworkName, []string{defaultPodRange}),
		...
		},
		{
			...
			defaultParamSet: newL3GNP(networkv1.DefaultPodNetworkName, []string{defaultPodRange, newPodRange2}),
			nodeList:        []*v1.Node{node1},
			expectedRanges:  []string{defaultPodRange, newPodRange1},
		},
		{
			name:            "PodIPv4Range order does not matter",
			defaultParamSet: newL3GNP(networkv1.DefaultPodNetworkName, []string{newPodRange1, defaultPodCIDR}),
		...
		},
		{
			...
			defaultParamSet:    newL3GNP(networkv1.DefaultPodNetworkName, []string{defaultPodRange}),
			nonDefaultParamSet: newL3GNP(gkeNetworkParamSetName, []string{newPodRange1}),
...
// newGNP returns paramset object with default vpc and subnet
func newL3GNP(name string, rangeNames []string) *networkv1.GKENetworkParamSet {
