...
func NewGKENetworkParamSetController(
	...
	nodeInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			...
			if newNode.Spec.PodCIDR != "" && newNode.Spec.PodCIDR != oldNode.Spec.PodCIDR {
				if c.nonDefaultParamsPodRanges(newNode) {
					c.queue.Add(networkv1.DefaultPodNetworkName)
				}
			}
		},
		DeleteFunc: func(obj interface{}) {
			node := obj.(*v1.Node)
			if v, ok := node.Labels[utilnode.NodePoolPodRangeLabelPrefix]; ok {
...
func (c *Controller) reconcile(ctx context.Context, key string) error {
	...
	// always re-create "default" paramset to unsure the valid vpc, subnet and cluster-default pod range
	if params.Name == networkv1.DefaultPodNetworkName {
		if err = c.setDesiredDefaultParamSet(ctx, params); err != nil {
			return err
		}
	}
...
// setDesiredDefaultParamSet set the "default" params to desired state
func (c *Controller) setDesiredDefaultParamSet(ctx context.Context, params *networkv1.GKENetworkParamSet) error {
	// get vpc
	...
	if v, ok := params.Labels[labelsAddonManagerMode]; !ok || v != addonManagerMode {
		params.Labels[labelsAddonManagerMode] = addonManagerMode
	}
